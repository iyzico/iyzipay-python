...
class PayoutCompletedTransactionList(IyzipayResource):
    ...
    def retrieve(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/reporting/settlement/payoutcompleted",
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(PayoutCompletedTransactionList, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(super(PayoutCompletedTransactionList, cls).to_pki_request_string(request)) \
