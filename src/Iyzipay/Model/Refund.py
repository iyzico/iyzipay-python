...
class Refund(IyzipayResource):
    ...
    def create(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/payment/iyzipos/refund",
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(Refund, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(
            super(Refund, cls).to_pki_request_string(request)) \
