...
class Cancel(IyzipayResource):
    ...
    def create(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/payment/iyzipos/cancel",
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(Cancel, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(super(Cancel, cls).to_pki_request_string(request)) \
            .append("paymentId", request.get('payment_id', None)) \
            .append("ip", request('ip', None)) \
            .get_request_string()
