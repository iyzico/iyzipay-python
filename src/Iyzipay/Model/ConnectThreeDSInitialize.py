...
class ConnectThreeDSInitialize(IyzipayResource):
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(ConnectThreeDSInitialize, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(
            super(ConnectThreeDSInitialize, cls).to_pki_request_string(request)) \
