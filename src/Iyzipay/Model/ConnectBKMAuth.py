...
class ConnectBKMAuth(IyzipayResource):
    ...
    def retrieve(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/payment/iyziconnect/bkm/auth/detail",
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(ConnectBKMAuth, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(
            super(ConnectBKMAuth, cls).to_pki_request_string(request)) \
