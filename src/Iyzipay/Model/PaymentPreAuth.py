...
class PaymentPreAuth(IyzipayResource):
    ...
    def create(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/payment/iyzipos/preauth/ecom",
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(PaymentPreAuth, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(
            super(PaymentPreAuth, cls).to_pki_request_string(request)) \
