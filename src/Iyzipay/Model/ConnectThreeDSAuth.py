...
class ConnectThreeDSAuth(IyzipayResource):
    ...
    def create(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/payment/iyziconnect/auth3ds",
    ...
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(ConnectThreeDSAuth, cls).get_json_object(request)) \
            .add("paymentId", request.get('payment_id', None)) \
            .add("conversationData", request.get('conversation_data', None)) \
            .get_object()
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(
            super(ConnectThreeDSAuth, cls).to_pki_request_string(request)) \
            .append("paymentId", request.get('payment_id', None)) \
            .append("conversationData", request.get('conversation_data', None)) \
            .get_request_string()