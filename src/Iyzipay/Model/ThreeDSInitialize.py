...
class ThreeDSInitialize(IyzipayResource):
    ...
    def create(cls, request, options):
        ...
        raw_result = HttpClient.create().post(options['base_url'] + "/payment/iyzipos/initialize3ds/ecom",
                                              IyzipayResource.get_http_header(pki_string_request, options),
                                              json_encoded_request)
        json_result = raw_result.json()
        if 'threeDSHtmlContent' in json_result.keys():
            json_result['threeDSHtmlContent'] = base64.b64decode(json_result['threeDSHtmlContent'])
        return json_result

    @classmethod
    def get_json_object(cls, request):
        json_object = JsonBuilder.from_json_object(super(ThreeDSInitialize, cls).get_json_object(request)) \
    ...
    def to_pki_request_string(cls, request):
        return RequestStringBuilder.create().append_super(
            super(ThreeDSInitialize, cls).to_pki_request_string(request)) \
